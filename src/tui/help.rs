...
const PT_PAGE_0: &[HelpRow<'static>] = &[
    hdr("NAVEGACAO"),
    key("m", "abre o menu de paineis"),
    key(
        "1-7",
        "abre o painel em tela cheia; repetir volta ao dashboard",
    ),
    ...
    key("← →", "com PROCESSOS focado, rola comando horizontalmente"),
    key("y / Enter", "copia comando completo do processo focado"),
    key("Enter", "com CORES focado filtra nucleo; Esc remove"),
    txt(""),
    hdr("ACOES GERAIS"),
...
const EN_PAGE_0: &[HelpRow<'static>] = &[
    hdr("NAVIGATION"),
    ...
    key("← →", "with PROCESSES focused, scroll command horizontally"),
    key("y / Enter", "copy full command of focused process"),
    key("Enter", "with CORES focused filter by core; Esc clears"),
    txt(""),
    hdr("GENERAL ACTIONS"),
...
const PT_PAGE_1: &[HelpRow<'static>] = &[
        hdr("CPU: VISAO GERAL"),
...
const EN_PAGE_1: &[HelpRow<'static>] = &[
        hdr("CPU: OVERVIEW"),
...
const PT_PAGE_2: &[HelpRow<'static>] = &[
    hdr("4:MEM: PAINEL DE MEMORIA"),
    key(
        "used",
        "RAM usada por aplicativos; nao inclui cache/buffers",
    ),
...
const EN_PAGE_2: &[HelpRow<'static>] = &[
...
const PT_PAGE_3: &[HelpRow<'static>] = &[
    hdr("5:DISKS: ESPACO"),
...
const EN_PAGE_3: &[HelpRow<'static>] = &[
    hdr("5:DISKS: SPACE"),
...
const PT_PAGE_4: &[HelpRow<'static>] = &[
    hdr("IOPS"),
...
const EN_PAGE_4: &[HelpRow<'static>] = &[
    hdr("IOPS"),
...
const PT_PAGE_5: &[HelpRow<'static>] = &[
...
const EN_PAGE_5: &[HelpRow<'static>] = &[
...
/// Help pages in order, paired with the title shown in the box.
const PAGES_PT: [(&str, &[HelpRow<'static>]); 6] = [
    ("help 1/6: teclas", PT_PAGE_0),
    ("help 2/6: bloco CPU", PT_PAGE_1),
    ("help 3/6: memoria e discos", PT_PAGE_2),
    ("help 4/6: discos e rede", PT_PAGE_3),
    ("help 5/6: IO: o que cada stat significa", PT_PAGE_4),
    ("help 6/6: historico e gravacao", PT_PAGE_5),
];

const PAGES_EN: [(&str, &[HelpRow<'static>]); 6] = [
    ("help 1/6: keys", EN_PAGE_0),
    ("help 2/6: CPU block", EN_PAGE_1),
    ("help 3/6: memory and disks", EN_PAGE_2),
    ("help 4/6: disks and network", EN_PAGE_3),
    ("help 5/6: IO: what each stat means", EN_PAGE_4),
    ("help 6/6: history and recordings", EN_PAGE_5),
];

/// Renders one help page. An out-of-range index falls back to the first page
/// rather than panicking: `help_page` is clamped by the caller, so this only
/// guards against a future caller that forgets to.
pub fn page(page: usize, lang: Lang, theme: &Theme) -> (String, Vec<Line<'static>>) {
    let pages = match lang {
        Lang::Pt => &PAGES_PT,
        Lang::En => &PAGES_EN,
    };
    let (title, rows) = pages.get(page).copied().unwrap_or(pages[0]);
    (title.to_string(), help_lines(rows, theme))
}
